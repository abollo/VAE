...
class UNet(object):
    def __init__(self, experiment_dir=None, experiment_id=0, batch_size=16, input_width=256, output_width=256,
        ...
        if experiment_dir:
            ...
            self.checkpoint_dir = os.path.join(self.experiment_dir, "checkpoint_20")
            self.sample_dir = os.path.join(self.experiment_dir, "sample_20")
            self.log_dir = os.path.join(self.experiment_dir, "logs_20")
    ...
    def gaussion_encoder(self, images, is_training, reuse=False):
        with tf.variable_scope("generator"):
            ...
            e1 = batch_norm(images2, is_training, scope="gaussion_images2")

            e2 = encode_layer(e1, self.generator_dim2 * 2, 2)
            e3 = encode_layer(e2, self.generator_dim2 * 4, 3)
            e3 = tf.nn.max_pool(e3,[1,2,2,1],[1,1,1,1],padding="SAME")
            e4 = encode_layer(e3, self.generator_dim2 * 8, 4)
            e5 = encode_layer(e4, self.generator_dim2 * 8, 5)
            e6 = encode_layer(e5, self.generator_dim2 * 8, 6)
            e6 = tf.nn.max_pool(e6, [1, 2, 2, 1], [1, 1, 1, 1], padding="SAME")
            e7 = encode_layer(e6, self.generator_dim2 * 8, 7)
            e8 = encode_layer(e7, self.generator_dim2 * 8, 8)
    ...
    def generator_gaussian(self, enc_layers, images_target, embedding_ids, inst_norm, is_training, reuse=False):
        ...
        e8_gaussion = self.gaussion_encoder(images_target, is_training=is_training, reuse=reuse)
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        target_gaussian1 = self.gaussion_encoder(real_A,  is_training=is_training,
                                                         reuse=False)

        source_gaussian1 = self.gaussion_encoder(real_B,  is_training=is_training,
                                                 reuse=True)

        target_gaussian2 = self.e_decoder(target_gaussian1, reuse=False)
        source_gaussian2 = self.e_decoder(source_gaussian1, reuse=True)
        # local_embeddings = tf.nn.embedding_lookup(embeddings, ids=embedding_ids)
        # local_embeddings = tf.reshape(local_embeddings, [self.batch_size, 1, 1, self.embedding_dim])
        embedded_sum_st = tf.concat([layers_source["e8"], target_gaussian2], 3)
        embedded_sum_ts = tf.concat([layers_target["e8"], source_gaussian2], 3)

        embedded_sum_ss = tf.concat([layers_source["e8"], source_gaussian2], 3)
        embedded_sum_tt = tf.concat([layers_target["e8"], target_gaussian2], 3)
        # print(e8.get_shape())
        # print(embedded.get_shape().as_list())
        fake_target_st = self.decoder(embedded_sum_st, layers_source, embedding_ids, inst_norm, is_training=is_training, reuse=False)
        fake_target_ts = self.decoder(embedded_sum_ts, layers_target, embedding_ids, inst_norm, is_training=is_training,
                                      reuse=True)

        fake_target_tt = self.decoder(embedded_sum_tt, layers_target, embedding_ids, inst_norm, is_training=is_training,
                                      reuse=True)
        fake_target_ss = self.decoder(embedded_sum_ss, layers_source, embedding_ids, inst_norm, is_training=is_training,
                                      reuse=True)

        st_e8, layers_st = self.encoder(fake_target_st, is_training=is_training,reuse=True)

        ts_e8, layers_ts = self.encoder(fake_target_ts, is_training=is_training, reuse=True)
        tt_e8, layers_tt = self.encoder(fake_target_tt, is_training=is_training, reuse=True)
        ss_e8, layers_ss = self.encoder(fake_target_ss, is_training=is_training, reuse=True)

        st_gaussian1 = self.gaussion_encoder(fake_target_st, is_training=is_training,
                                                 reuse=True)

        ts_gaussian1 = self.gaussion_encoder(fake_target_ts, is_training=is_training,
                                             reuse=True)

        ss_gaussian1 = self.gaussion_encoder(fake_target_ss, is_training=is_training,
                                             reuse=True)
        tt_gaussian1 = self.gaussion_encoder(fake_target_tt, is_training=is_training,
                                             reuse=True)

        l1_loss_ss = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_ss - real_B), [1, 2, 3])
        l1_loss_ss2 = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_ts - real_B), [1, 2, 3])
        l1_loss_ss3 = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_ts - fake_target_ss), [1, 2, 3])

        l1_loss_tt = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_tt - real_A), [1, 2, 3])
        l1_loss_tt2 = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_st - real_A), [1, 2, 3])
        l1_loss_tt3 = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_st - fake_target_tt), [1, 2, 3])

        const_loss_tt = tf.reduce_mean(tf.abs(tt_e8 - target_e8), [1, 2, 3])
        const_loss_st = tf.reduce_mean(tf.abs(st_e8 - target_e8), [1, 2, 3])

        const_loss_ss = tf.reduce_mean(tf.abs(ss_e8 - source_e8), [1, 2, 3])
        const_loss_ts = tf.reduce_mean(tf.abs(ts_e8 - target_e8), [1, 2, 3])

        gaussian_loss_tt = tf.reduce_mean(tf.abs(tt_gaussian1 - target_gaussian1), [1])
        gaussian_loss_st = tf.reduce_mean(tf.abs(st_gaussian1 - target_gaussian1), [1])

        gaussian_loss_ss = tf.reduce_mean(tf.abs(ss_gaussian1 - source_gaussian1), [1])
        gaussian_loss_ts = tf.reduce_mean(tf.abs(ts_gaussian1 - source_gaussian1), [1])
        ...
        #real_category_loss=tf.reduce_sum(tf.abs(target_gaussian1_fake[:, :self.latent_dim] - e8_gaussion_sum[:, :self.latent_dim]))
        ...
        q_z_s = distributions.Normal(loc=source_gaussian1[:, :self.latent_dim],
                                   scale=tf.nn.softplus(source_gaussian1[:, self.latent_dim:]))
        ...
        kl_loss_s = tf.reduce_sum(distributions.kl_divergence(q_z_s, p_z), 1)
        ...
        T_loss =  tf.reduce_sum(kl_loss+kl_loss_s+gaussian_loss_tt+gaussian_loss_st+gaussian_loss_ts+gaussian_loss_ss+const_loss_tt+const_loss_st+const_loss_ts+const_loss_ss+l1_loss_ss+l1_loss_ss2+l1_loss_ss3+l1_loss_tt+l1_loss_tt2+l1_loss_tt3)
        ...
        l1_loss_summary = tf.summary.scalar("l1_loss", tf.reduce_sum(l1_loss_ss))

        l1_loss_tt_summary = tf.summary.scalar("l1_loss_tt", tf.reduce_sum(l1_loss_tt))


        kl_loss_summary = tf.summary.scalar("kl_loss", tf.reduce_sum(kl_loss))
        kl_loss_s_summary = tf.summary.scalar("kl_loss_s", tf.reduce_sum(kl_loss_s))
        ...
        All_merged_summary = tf.summary.merge([l1_loss_summary,l1_loss_tt_summary,T_loss_summary,kl_loss_summary,kl_loss_s_summary])

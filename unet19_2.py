...
class UNet(object):
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        l1_loss_ss = self.L1_penalty * tf.reduce_mean(tf.abs(fake_source - real_B), [1, 2, 3])
        ...
        q_z_ss = distributions.Normal(loc=source_gaussian1[:, :self.latent_dim],
        ...
        kl_loss_ss = tf.reduce_sum(distributions.kl_divergence(q_z_ss, p_z), 1)
        ...
        l1_loss_ss_summary = tf.summary.scalar("l1_loss_ss", tf.reduce_sum(l1_loss_ss))

        kl_loss_ss_summary = tf.summary.scalar("kl_loss_ss", tf.reduce_sum(kl_loss_ss))
        ...
        All_merged_summary = tf.summary.merge([l1_loss_summary,T_loss_summary,kl_loss_summary,const_loss_summary,real_category_loss_summary,l1_loss_ss_summary,kl_loss_ss_summary])

...
class UNet(object):
    ...
    def gaussion_encoder(self, images_source, images_target,is_training, reuse=False):
        with tf.variable_scope("generator"):
            ...
            def encode_layer(x, output_filters, layer):
                ...
                return enc
            images=tf.concat([images_source,images_target],3)
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        target_gaussian1_shuffle = self.gaussion_encoder(real_A_shuffle, real_B, is_training=is_training,
                                                         reuse=False)

        real_A_shuffle2=tf.random_shuffle(real_A_shuffle)
        target_gaussian2_shuffle = self.gaussion_encoder(real_A_shuffle2, real_B, is_training=is_training,
                                                      reuse=True)

        real_A_shuffle3=tf.random_shuffle(real_A_shuffle2)
        target_gaussian3_shuffle = self.gaussion_encoder(real_A_shuffle3, real_B, is_training=is_training,
                                                         reuse=True)

        real_A_shuffle4=tf.random_shuffle(real_A_shuffle3)

        target_gaussian4_shuffle = self.gaussion_encoder(real_A_shuffle4, real_B, is_training=is_training,
                                                         reuse=True)

        real_A_shuffle5 = tf.random_shuffle(real_A_shuffle4)

        target_gaussian5_shuffle = self.gaussion_encoder(real_A_shuffle5, real_B, is_training=is_training,
                                                         reuse=True)
        ...
        const_loss8 = tf.reduce_mean(tf.square(layers_source_fake["e8"] - layers_source["e8"]), [1, 2, 3]) / 256.0
        const_loss7 = tf.reduce_mean(tf.square(layers_source_fake["e7"] - layers_source["e7"]), [1, 2, 3]) / (2.0 * 2.0 * 256.0)
        const_loss6 = tf.reduce_mean(tf.square(layers_source_fake["e6"] - layers_source["e6"]), [1, 2, 3]) / (4.0 * 4.0 * 256.0)
        const_loss5 = tf.reduce_mean(tf.square(layers_source_fake["e5"] - layers_source["e5"]), [1, 2, 3]) / (8.0 * 8.0 * 256.0)
        const_loss4 = tf.reduce_mean(tf.square(layers_source_fake["e4"] - layers_source["e4"]), [1, 2, 3]) / (
                    16.0 * 16.0 * 256.0)
        const_loss3 = tf.reduce_mean(tf.square(layers_source_fake["e3"] - layers_source["e3"]), [1, 2, 3]) / (
                    32.0 * 32.0 * 128.0)
        const_loss2 = tf.reduce_mean(tf.square(layers_source_fake["e2"] - layers_source["e2"]), [1, 2, 3]) / (
                    64.0 * 64.0 * 64.0)
        const_loss1 = tf.reduce_mean(tf.square(layers_source_fake["e1"] - layers_source["e1"]), [1, 2, 3]) / (
                    128.0 * 128.0 * 32)
        ...
        T_loss =  tf.reduce_mean(tf.reduce_sum(l1_loss +kl_loss))

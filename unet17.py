...
LossHandle = namedtuple("LossHandle", ["l1_loss","kl_loss","T_loss","T_loss2","T_loss3","T_loss4"])
...
class UNet(object):
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        fake_target_shuffle2, target_gaussian1_shuffle2, target_gaussian2_shuffle2 = self.generator_gaussian(layers_source,
                                                                                                          real_A_shuffle2,
                                                                                                          embedding_ids,
        ...
        fake_target_shuffle3, target_gaussian1_shuffle3, target_gaussian2_shuffle3 = self.generator_gaussian(layers_source,
                                                                                                          real_A_shuffle3,
                                                                                                          embedding_ids,
                                                                                                          is_training=is_training,
                                                                                                          inst_norm=inst_norm,
        ...
        l1_loss_summary2 = tf.summary.scalar("l1_loss2", tf.reduce_sum(l1_loss2))
        l1_loss_summary3 = tf.summary.scalar("l1_loss3", tf.reduce_sum(l1_loss3))

        l1_loss_summary4 = tf.summary.scalar("l1_loss4", tf.reduce_sum(l1_loss4))

        kl_loss_summary = tf.summary.scalar("kl_loss", kl_loss_1)
        kl_loss2_summary = tf.summary.scalar("kl_loss2", tf.reduce_sum(kl_loss2))
        kl_loss3_summary = tf.summary.scalar("kl_loss3", tf.reduce_sum(kl_loss3))
        kl_loss4_summary = tf.summary.scalar("kl_loss4", tf.reduce_sum(kl_loss4))
        ...
        All_merged_summary = tf.summary.merge([l1_loss_summary,l1_loss_summary2,l1_loss_summary3,l1_loss_summary4,T_loss_summary,kl_loss_summary,kl_loss2_summary,kl_loss3_summary,kl_loss4_summary,const_loss_summary,real_category_loss_summary])
        ...
        loss_handle = LossHandle(
                                 T_loss=T_loss,
            T_loss2=T_loss2,T_loss3=T_loss3,T_loss4=T_loss4,
    ...
    def train(self, lr=0.0002, epoch=100, schedule=10, resume=True, flip_labels=False,
        ...
        all_optimizer2 = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(loss_handle.T_loss2, var_list=gaussian_vars)
        all_optimizer3 = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(loss_handle.T_loss3, var_list=gaussian_vars)
        all_optimizer4 = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(loss_handle.T_loss4, var_list=gaussian_vars)

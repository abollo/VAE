...
LossHandle = namedtuple("LossHandle", ["l1_loss","kl_loss","T_loss","T_loss2","T_loss3","T_loss4","T_loss5"])
...
class UNet(object):
    ...
    def gaussion_encoder(self, images, encoding_layers,is_training, reuse=False):
        with tf.variable_scope("gaussion_encoders"):
            if reuse:
    ...
    def gaussion_encoder2(self, images, encoding_layers,is_training, reuse=False):
        with tf.variable_scope("gaussion_encoders"):
            if reuse:
                tf.get_variable_scope().reuse_variables()
            def encode_layer(x, output_filters, layer,enc_layer):
                act = lrelu(x)
                conv = conv2d(act, output_filters=output_filters, scope="gaussion22_e%d" % layer)
                enc = batch_norm(conv, is_training, scope="g22_e%d_bn" % layer)
                dec = tf.concat([enc, enc_layer], 3)
                #encode_layers["e%d" % layer] = enc
                return dec
            images1=lrelu(images)
    ...
    def gaussion_encoder3(self, images, is_training, reuse=False):
        with tf.variable_scope("gaussion_encoders"):
            ...
            def encode_layer(x, output_filters, layer):
                act = lrelu(x)
                conv = conv2d(act, output_filters=output_filters, scope="gaussion2_e%d" % layer)
                enc = batch_norm(conv, is_training, scope="g2_e%d_bn" % layer)
                #encode_layers["e%d" % layer] = enc
                return enc

            e1 = conv2d(images, self.generator_dim, scope="gaussion2_e1")
            ...
            e9 = tf.reshape(e8, [-1, nodes])
            # print(e8.get_shape())
            gaussian_params = fc(e9, self.latent_dim * 2, stddev=0.02, scope="gaussion2_fc")
            gaussian_params_act=lrelu(gaussian_params)
    ...
    def encoder2(self, images, is_training, reuse=False):
        with tf.variable_scope("generator"):
            if reuse:
                tf.get_variable_scope().reuse_variables()
            ...
            def encode_layer(x, output_filters, layer):
                act = lrelu(x)
                conv = conv2d(act, output_filters=output_filters, scope="g2_e%d_conv" % layer)
                enc = batch_norm(conv, is_training, scope="g2_e%d_bn" % layer)
                encode_layers["e%d" % layer] = enc
                return enc

            e1 = conv2d(images, self.generator_dim, scope="g2_e1_conv")
            encode_layers["e1"] = e1
            e2 = encode_layer(e1, self.generator_dim * 2, 2)
            e3 = encode_layer(e2, self.generator_dim * 4, 3)
            e4 = encode_layer(e3, self.generator_dim * 8, 4)
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        fake_target, target_gaussian1,target_gaussian2,source_e8,layers_source = self.generator(real_B, real_A, embedding_ids, is_training=is_training,inst_norm=inst_norm)
        ...
        source_fake_e8, layers_source_fake = self.encoder2(fake_target_shuffle, is_training=is_training)


        target_gaussian1_fake=self.gaussion_encoder2(fake_target_shuffle, layers_source,is_training=is_training)
        ...
        l1_loss2 = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_shuffle2 - real_A), [1, 2, 3])
        l1_loss3 = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_shuffle3 - real_A), [1, 2, 3])
        l1_loss4 = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_shuffle4 - real_A), [1, 2, 3])
        l1_loss5 = self.L1_penalty * tf.reduce_mean(tf.abs(fake_target_shuffle5 - real_A), [1, 2, 3])
        ...
        All_merged_summary = tf.summary.merge([l1_loss_summary,l1_loss2_summary,T_loss_summary,kl_loss_summary,const_loss_summary,real_category_loss_summary,T_loss2_summary,T_loss3_summary,T_loss4_summary,T_loss5_summary])
        ...
        loss_handle = LossHandle(
                                 T_loss=T_loss,
            T_loss2=T_loss2,T_loss3=T_loss3,T_loss4=T_loss4,T_loss5=T_loss5,
    ...
    def train(self, lr=0.0002, epoch=100, schedule=10, resume=True, flip_labels=False,
        ...
        all_vars = self.retrieve_trainable_vars(freeze_encoder=freeze_encoder)
        input_handle, loss_handle, eval_handle, summary_handle = self.retrieve_handles()
        ...
        all_optimizer2 = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(loss_handle.T_loss2, var_list=all_vars)
        all_optimizer3 = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(loss_handle.T_loss3, var_list=all_vars)
        all_optimizer4 = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(loss_handle.T_loss4, var_list=all_vars)
        all_optimizer5 = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(loss_handle.T_loss5, var_list=all_vars)
        ...
        for ei in range(epoch):
            ...
            for bid, batch in enumerate(train_batch_iter):
                ...
                _, _,_,_,_,l1_loss_run, kl_loss_run, T_loss_run,T_sum_run,gaussian_params= self.sess.run([all_optimizer,all_optimizer2,all_optimizer3,all_optimizer4,all_optimizer5,loss_handle.l1_loss, loss_handle.kl_loss,loss_handle.T_loss,summary_handle.T_sum,eval_handle.gaussian_params],feed_dict={
